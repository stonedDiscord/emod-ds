...
int main(int argc, char *argv[])
{
    ...
    // V_COUNT IRQ is called at scanline 80 (mid-frame)
    SetYtrigger(80);
    ...
    while (!exit_loop)
    {
        ...
        if (physicsEngine.getState() == PE_RUNNING)
        {
            // simulate
            physicsEngine.simulate();
            syncPhysicsEngine(physicsEngine);
        }

        listenPhysicsEngineRequests(physicsEngine);
